...
type OnBlurType = Record<'id' | 'target' | 'current' | 'value', string> & { index: number }

const ManageBanner = () => {
	const [banners, setBanner, initBanner] = useStateArray<BannerType>()
	const onBlur = async ({ current, id, target, value, index }: OnBlurType) => {
		setBanner({ ...banners[index], [target]: value }, index)
		if (current !== value) await manageBanner({ id, target, value, type: 'change' })
	}
	const getData = async () => {
	...
	return <Container>
		...
		<DragSortable
			...
			renderItem={({ item: { visible, id, btnText, image, redirect }, index }) => {
